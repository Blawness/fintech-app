...
const RobustLineChart: React.FC<RobustLineChartProps> = ({ product, className = '' }) => {
  ...
  const initializeChart = useCallback(() => {
    console.log('=== INITIALIZE ROBUST LINE CHART START ===')
    ...
    if (!isVisible) {
      console.log('Chart not visible yet, skipping initialization')
      return
    }
    ...
    try {
      console.log('Initializing robust line chart...')
      ...
      console.log('Robust line chart initialized successfully')

    } catch (err) {
      console.error('Error initializing robust line chart:', err)
  ...
  const updateChart = useCallback(() => {
    if (!lineSeriesRef.current || chartData.length === 0) {
      console.log('Line series not ready or no data')
      return
    }

    try {
      console.log('Updating robust line chart with data:', chartData.length, 'points')
      ...
      console.log('Robust line chart updated successfully')
      
    } catch (err) {
      console.error('Error updating robust line chart:', err)
  ...
  const cleanupChart = useCallback(() => {
    if (chartRef.current) {
      try {
        ...
        console.log('Robust line chart cleaned up')
      } catch (err) {
        console.error('Error cleaning up robust line chart:', err)
      }
    }
  }, [])
  ...
  useEffect(() => {
    if (isVisible && !chartRef.current) {
      console.log('Chart is visible, initializing...')
  ...
  useEffect(() => {
    console.log('=== ROBUST LINE CHART UPDATE EFFECT ===')
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gray-50 rounded">
              <div className="text-center">
                ...
                <p className="text-gray-600 text-sm">
                  {isVisible ? 'Initializing chart...' : 'Waiting for chart to be visible...'}
                </p>
                {process.env.NODE_ENV === 'development' && (
                  <p className="text-xs text-gray-500 mt-1">
                    Visible: {isVisible ? 'Yes' : 'No'} | Container: {chartContainerRef.current ? 'Ready' : 'Not Ready'}
                  </p>
                )}
              </div>
            </div>
...
export default RobustLineChart
