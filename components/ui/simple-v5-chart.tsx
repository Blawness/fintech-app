...
const SimpleV5Chart: React.FC<SimpleV5ChartProps> = ({ product, className = '' }) => {
  ...
  const fetchChartData = useCallback(async (hours: number) => {
    ...
    try {
      console.log(`[SimpleV5Chart] Fetching data for ${product.name}, hours: ${hours}`)
      ...
      console.log(`[SimpleV5Chart] Data received:`, result.data.chartData.length, 'points')
      setChartData(result.data.chartData)
      
    } catch (err) {
      console.error('[SimpleV5Chart] Fetch error:', err)
  ...
  const waitForContainer = useCallback((): Promise<HTMLDivElement> => {
    return new Promise((resolve, reject) => {
      ...
      const checkContainer = () => {
        attempts++
        console.log(`[SimpleV5Chart] Container check attempt ${attempts}/${maxAttempts}`)
        
        if (chartContainerRef.current) {
          console.log('[SimpleV5Chart] Container found!')
          resolve(chartContainerRef.current)
        } else if (attempts >= maxAttempts) {
          console.error('[SimpleV5Chart] Container not found after max attempts')
          reject(new Error('Container not found after maximum attempts'))
        } else {
          console.log('[SimpleV5Chart] Container not ready, retrying...')
  ...
  // Initialize chart with minimal v5.0 API
  const initializeChart = useCallback(async () => {
    console.log('[SimpleV5Chart] === INITIALIZE START ===')
    
    if (chartRef.current) {
      console.log('[SimpleV5Chart] Chart already exists, skipping')
      return
    }
    ...
    try {
      ...
      console.log('[SimpleV5Chart] Creating chart...')
      ...
      try {
        ...
        console.log('[SimpleV5Chart] Line series styled successfully')
        
      } catch (lineError) {
        console.error('[SimpleV5Chart] Line series error:', lineError)
      ...
      const handleResize = () => {
        if (chartContainerRef.current && chartRef.current) {
          try {
            ...
            console.warn('[SimpleV5Chart] Resize warning:', resizeError)
          }
        }
      }
      ...
      console.log('[SimpleV5Chart] === INITIALIZE COMPLETE ===')

    } catch (err) {
      console.error('[SimpleV5Chart] Initialize error:', err)
  ...
  const updateChart = useCallback(() => {
    console.log('[SimpleV5Chart] === UPDATE CHART START ===')
    console.log('[SimpleV5Chart] lineSeriesRef.current:', lineSeriesRef.current)
    console.log('[SimpleV5Chart] chartData.length:', chartData.length)
    
    if (!lineSeriesRef.current) {
      console.log('[SimpleV5Chart] No line series available, cannot update')
      return
    }
    
    if (chartData.length === 0) {
      console.log('[SimpleV5Chart] No data available, cannot update')
      return
    }

    try {
      console.log('[SimpleV5Chart] Updating chart with', chartData.length, 'data points')
      ...
      console.log('[SimpleV5Chart] Valid data points:', uniqueData.length)

      if (uniqueData.length === 0) {
        console.warn('[SimpleV5Chart] No valid data to display')
        return
      }
      ...
      console.log('[SimpleV5Chart] Chart updated successfully')
      console.log('[SimpleV5Chart] === UPDATE CHART COMPLETE ===')
      
    } catch (err) {
      console.error('[SimpleV5Chart] Update error:', err)
  ...
  const cleanupChart = useCallback(() => {
    if (chartRef.current) {
      try {
        ...
        console.log('[SimpleV5Chart] Chart cleaned up')
      } catch (err) {
        console.error('[SimpleV5Chart] Cleanup error (non-critical):', err)
        // Reset refs anyway
        chartRef.current = null
        lineSeriesRef.current = null
        setIsChartReady(false)
  ...
  useEffect(() => {
    console.log('[SimpleV5Chart] === MOUNT EFFECT ===')
  ...
  useEffect(() => {
    console.log('[SimpleV5Chart] Timeframe changed to:', selectedTimeframe)
  ...
  useEffect(() => {
    console.log('[SimpleV5Chart] === DATA UPDATE EFFECT ===')
    console.log('[SimpleV5Chart] isChartReady:', isChartReady)
    console.log('[SimpleV5Chart] chartData.length:', chartData.length)
    console.log('[SimpleV5Chart] lineSeriesRef.current:', lineSeriesRef.current)
    
    if (isChartReady && chartData.length > 0 && lineSeriesRef.current) {
      console.log('[SimpleV5Chart] Updating chart with new data...')
      updateChart()
    }
      ...
      <div className="p-6 border-b bg-gradient-to-r from-blue-50 to-indigo-50">
        ...
        <div className="flex gap-3 items-center">
          ...
          {process.env.NODE_ENV === 'development' && (
            <button
              onClick={() => {
                console.log('[SimpleV5Chart] Manual update triggered')
                if (chartData.length > 0) {
                  updateChart()
                } else {
                  const hours = timeframes.find(tf => tf.value === selectedTimeframe)?.hours || 24
