...
const WorkingLineChart: React.FC<WorkingLineChartProps> = ({ product, className = '' }) => {
  ...
  const fetchChartData = useCallback(async (hours: number) => {
    ...
    try {
      console.log(`[WorkingChart] Fetching data for ${product.name}, hours: ${hours}`)
      ...
      console.log(`[WorkingChart] Data received:`, result.data.chartData.length, 'points')
      setChartData(result.data.chartData)
      
    } catch (err) {
      console.error('[WorkingChart] Fetch error:', err)
  ...
  // Initialize chart - using correct v5 API
  const initializeChart = useCallback(() => {
    console.log('[WorkingChart] === INITIALIZE START ===')
    console.log('[WorkingChart] Container:', chartContainerRef.current)
    console.log('[WorkingChart] Existing chart:', chartRef.current)
    
    if (!chartContainerRef.current) {
      console.error('[WorkingChart] No container found')
      return
    }

    if (chartRef.current) {
      console.log('[WorkingChart] Chart already exists, skipping')
      return
    }
    ...
    try {
      ...
      console.log('[WorkingChart] Creating chart with dimensions:', container.clientWidth, 'x', container.clientHeight)
      
      // Create chart with proper configuration for v5
      ...
      console.log('[WorkingChart] Chart created successfully')
      chartRef.current = chart

      // Add line series using correct v5 API
      console.log('[WorkingChart] Adding line series...')
      ...
      console.log('[WorkingChart] Line series created')
      ...
      console.log('[WorkingChart] === INITIALIZE COMPLETE ===')

    } catch (err) {
      console.error('[WorkingChart] Initialize error:', err)
  ...
  const updateChart = useCallback(() => {
    if (!lineSeriesRef.current || chartData.length === 0) {
      console.log('[WorkingChart] Cannot update - series or data not ready')
      return
    }

    try {
      console.log('[WorkingChart] Updating chart with', chartData.length, 'data points')
      ...
      console.log('[WorkingChart] Valid data points:', uniqueData.length)

      if (uniqueData.length === 0) {
        console.warn('[WorkingChart] No valid data to display')
        return
      }
      ...
      console.log('[WorkingChart] Chart updated successfully')
      
    } catch (err) {
      console.error('[WorkingChart] Update error:', err)
  ...
  const cleanupChart = useCallback(() => {
    if (chartRef.current) {
      try {
        ...
        console.log('[WorkingChart] Chart cleaned up')
      } catch (err) {
        console.error('[WorkingChart] Cleanup error:', err)
      }
    }
  }, [])
  ...
  useEffect(() => {
    console.log('[WorkingChart] === MOUNT EFFECT ===')
  ...
  useEffect(() => {
    console.log('[WorkingChart] === DATA UPDATE EFFECT ===')
    console.log('[WorkingChart] isChartReady:', isChartReady)
    console.log('[WorkingChart] chartData.length:', chartData.length)
      ...
      <div className="p-4 border-b">
        ...
        <div className="flex gap-2 items-center">
          ...
          {process.env.NODE_ENV === 'development' && (
            <button
              onClick={() => {
                console.log('[WorkingChart] Manual update triggered')
