...
const LineChart: React.FC<LineChartProps> = ({ product, className = '' }) => {
  ...
  const updateChart = useCallback(() => {
    ...
    try {
      console.log('Updating line chart with data:', chartData.length, 'points')
  ...
  const cleanupChart = useCallback(() => {
    if (chartRef.current) {
      try {
        ...
        console.log('Line chart cleaned up')
      } catch (err) {
        console.error('Error cleaning up line chart:', err)
      }
    }
  }, [])
  ...
  useEffect(() => {
    const checkContainer = () => {
      if (chartContainerRef.current && !chartRef.current) {
        console.log('Container is ready, initializing chart...')
        initializeChart()
    ...
    const timer = setTimeout(checkContainer, 300)
    
    return () => {
      clearTimeout(timer)
    }
  }, [product.id, initializeChart])

  // Initialize chart on mount
  useEffect(() => {
    console.log('=== LINE CHART INITIALIZATION EFFECT ===')
    console.log('Product ID:', product.id)
    console.log('Container ref in effect:', chartContainerRef.current)
    
    return () => {
      console.log('Cleaning up line chart')
      cleanupChart()
    }
  }, [product.id, cleanupChart])
  ...
  useEffect(() => {
    console.log('=== LINE CHART UPDATE EFFECT ===')
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gray-50 rounded">
              <div className="text-center">
                ...
                <p className="text-gray-600 text-sm">
                  {chartContainerRef.current ? 'Initializing chart...' : 'Preparing chart container...'}
                </p>
                {process.env.NODE_ENV === 'development' && (
