...
const WorkingV5Chart: React.FC<WorkingV5ChartProps> = ({ product, className = '' }) => {
  ...
  const fetchChartData = useCallback(async (hours: number) => {
    ...
    try {
      console.log(`[WorkingV5Chart] Fetching data for ${product.name}, hours: ${hours}`)
      ...
      console.log(`[WorkingV5Chart] Data received:`, result.data.chartData.length, 'points')
      setChartData(result.data.chartData)
      
    } catch (err) {
      console.error('[WorkingV5Chart] Fetch error:', err)
  ...
  const waitForContainer = useCallback((): Promise<HTMLDivElement> => {
    return new Promise((resolve, reject) => {
      ...
      const checkContainer = () => {
        attempts++
        console.log(`[WorkingV5Chart] Container check attempt ${attempts}/${maxAttempts}`)
        
        if (chartContainerRef.current) {
          console.log('[WorkingV5Chart] Container found!')
          resolve(chartContainerRef.current)
        } else if (attempts >= maxAttempts) {
          console.error('[WorkingV5Chart] Container not found after max attempts')
          reject(new Error('Container not found after maximum attempts'))
        } else {
          console.log('[WorkingV5Chart] Container not ready, retrying...')
  ...
  // Initialize chart with correct v5.0 API and container waiting
  const initializeChart = useCallback(async () => {
    console.log('[WorkingV5Chart] === INITIALIZE START ===')
    
    if (chartRef.current) {
      console.log('[WorkingV5Chart] Chart already exists, skipping')
      return
    }
    ...
    try {
      ...
      console.log('[WorkingV5Chart] Creating chart...')
      ...
      console.log('[WorkingV5Chart] Chart created successfully')
      chartRef.current = chart

      // Add line series using correct v5.0 API - based on documentation
      console.log('[WorkingV5Chart] Adding line series...')
      try {
        ...
        console.log('[WorkingV5Chart] Line series created successfully')
        lineSeriesRef.current = lineSeries
      } catch (lineError) {
        console.error('[WorkingV5Chart] Line series error:', lineError)
      ...
      console.log('[WorkingV5Chart] Adding histogram series...')
      try {
        const histogramSeries = chart.addSeries('Histogram', {
          color: '#26a69a',
          priceFormat: {
            type: 'volume',
          },
        ...
        console.log('[WorkingV5Chart] Histogram series created successfully')
        histogramSeriesRef.current = histogramSeries
      } catch (histogramError) {
        console.error('[WorkingV5Chart] Histogram series error:', histogramError)
        // Try alternative approach
        const histogramSeries = chart.addSeries('Histogram')
        histogramSeriesRef.current = histogramSeries
      }
      ...
      const handleResize = () => {
        if (chartContainerRef.current && chartRef.current) {
          try {
            ...
            console.warn('[WorkingV5Chart] Resize warning:', resizeError)
          }
        }
      }
      ...
      console.log('[WorkingV5Chart] === INITIALIZE COMPLETE ===')

    } catch (err) {
      console.error('[WorkingV5Chart] Initialize error:', err)
  ...
  const updateChart = useCallback(() => {
    console.log('[WorkingV5Chart] === UPDATE CHART START ===')
    console.log('[WorkingV5Chart] lineSeriesRef.current:', lineSeriesRef.current)
    console.log('[WorkingV5Chart] chartData.length:', chartData.length)
    
    if (!lineSeriesRef.current || !histogramSeriesRef.current) {
      console.log('[WorkingV5Chart] No series available, cannot update')
      return
    }
    
    if (chartData.length === 0) {
      console.log('[WorkingV5Chart] No data available, cannot update')
      return
    }

    try {
      console.log('[WorkingV5Chart] Updating chart with', chartData.length, 'data points')
      ...
      console.log('[WorkingV5Chart] Valid data points:', uniqueData.length)

      if (uniqueData.length === 0) {
        console.warn('[WorkingV5Chart] No valid data to display')
        return
      }
      ...
      console.log('[WorkingV5Chart] Chart updated successfully')
      console.log('[WorkingV5Chart] === UPDATE CHART COMPLETE ===')
      
    } catch (err) {
      console.error('[WorkingV5Chart] Update error:', err)
  ...
  const cleanupChart = useCallback(() => {
    if (chartRef.current) {
      try {
        ...
        console.log('[WorkingV5Chart] Chart cleaned up')
      } catch (err) {
        console.error('[WorkingV5Chart] Cleanup error (non-critical):', err)
        // Reset refs anyway
        chartRef.current = null
        lineSeriesRef.current = null
        histogramSeriesRef.current = null
  ...
  useEffect(() => {
    console.log('[WorkingV5Chart] === MOUNT EFFECT ===')
  ...
  useEffect(() => {
    console.log('[WorkingV5Chart] Timeframe changed to:', selectedTimeframe)
  ...
  useEffect(() => {
    console.log('[WorkingV5Chart] === DATA UPDATE EFFECT ===')
    console.log('[WorkingV5Chart] isChartReady:', isChartReady)
    console.log('[WorkingV5Chart] chartData.length:', chartData.length)
    console.log('[WorkingV5Chart] lineSeriesRef.current:', lineSeriesRef.current)
    
    if (isChartReady && chartData.length > 0 && lineSeriesRef.current) {
      console.log('[WorkingV5Chart] Updating chart with new data...')
      updateChart()
    }
      ...
      <div className="p-6 border-b bg-gradient-to-r from-blue-50 to-indigo-50">
        ...
        <div className="flex gap-3 items-center">
          ...
          {process.env.NODE_ENV === 'development' && (
            <button
              onClick={() => {
                console.log('[WorkingV5Chart] Manual update triggered')
                if (chartData.length > 0) {
                  updateChart()
                } else {
                  const hours = timeframes.find(tf => tf.value === selectedTimeframe)?.hours || 24
