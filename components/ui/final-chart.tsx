...
const FinalChart: React.FC<FinalChartProps> = ({ product, className = '' }) => {
  ...
  const fetchChartData = useCallback(async (hours: number) => {
    ...
    try {
      console.log(`[FinalChart] Fetching data for ${product.name}, hours: ${hours}`)
      ...
      console.log(`[FinalChart] Data received:`, result.data.chartData.length, 'points')
      setChartData(result.data.chartData)
      
    } catch (err) {
      console.error('[FinalChart] Fetch error:', err)
  ...
  const waitForContainer = useCallback((): Promise<HTMLDivElement> => {
    return new Promise((resolve, reject) => {
      ...
      const checkContainer = () => {
        attempts++
        console.log(`[FinalChart] Container check attempt ${attempts}/${maxAttempts}`)
        
        if (chartContainerRef.current) {
          console.log('[FinalChart] Container found!')
          setContainerStatus('ready')
          resolve(chartContainerRef.current)
        } else if (attempts >= maxAttempts) {
          console.error('[FinalChart] Container not found after max attempts')
          setContainerStatus('not-found')
          reject(new Error('Container not found after maximum attempts'))
        } else {
          console.log('[FinalChart] Container not ready, retrying...')
  ...
  const initializeChart = useCallback(async () => {
    console.log('[FinalChart] === INITIALIZE START ===')
    console.log('[FinalChart] Init attempts:', initAttemptsRef.current)
    
    if (initAttemptsRef.current >= maxInitAttempts) {
      console.error('[FinalChart] Max initialization attempts reached')
      setError('Chart initialization failed after maximum attempts')
      return
    }

    if (chartRef.current) {
      console.log('[FinalChart] Chart already exists, skipping')
      return
    }

    try {
      initAttemptsRef.current++
      console.log(`[FinalChart] Initialization attempt ${initAttemptsRef.current}`)
      ...
      console.log('[FinalChart] Creating chart with dimensions:', container.clientWidth, 'x', container.clientHeight)
      ...
      console.log('[FinalChart] Chart created successfully')
      chartRef.current = chart

      // Add line series using correct v5.0.8 API
      console.log('[FinalChart] Adding line series...')
      try {
        // Use addSeries with LineSeries type - correct v5.0.8 API
        const lineSeries = chart.addSeries(LineSeries, {
          color: '#26a69a',
          lineWidth: 2,
        })
        
        console.log('[FinalChart] Line series created successfully')
        lineSeriesRef.current = lineSeries
        
      } catch (seriesError) {
        console.error('[FinalChart] Error creating line series:', seriesError)
        throw seriesError
      }
      ...
      console.log('[FinalChart] === INITIALIZE COMPLETE ===')

    } catch (err) {
      console.error('[FinalChart] Initialize error:', err)
      
      if (initAttemptsRef.current < maxInitAttempts) {
        console.log('[FinalChart] Retrying initialization...')
        setTimeout(() => {
          initializeChart()
        }, 200)
      } else {
  ...
  const updateChart = useCallback(() => {
    if (!lineSeriesRef.current || chartData.length === 0) {
      console.log('[FinalChart] Cannot update - series or data not ready')
      return
    }

    try {
      console.log('[FinalChart] Updating chart with', chartData.length, 'data points')
      ...
      console.log('[FinalChart] Valid data points:', uniqueData.length)

      if (uniqueData.length === 0) {
        console.warn('[FinalChart] No valid data to display')
        return
      }
      ...
      console.log('[FinalChart] Chart updated successfully')
      
    } catch (err) {
      console.error('[FinalChart] Update error:', err)
  ...
  const cleanupChart = useCallback(() => {
    if (chartRef.current) {
      try {
        ...
        console.log('[FinalChart] Chart cleaned up')
      } catch (err) {
        console.error('[FinalChart] Cleanup error:', err)
      }
    }
  }, [])
  ...
  useEffect(() => {
    console.log('[FinalChart] === MOUNT EFFECT ===')
  ...
  useEffect(() => {
    console.log('[FinalChart] === DATA UPDATE EFFECT ===')
    console.log('[FinalChart] isChartReady:', isChartReady)
    console.log('[FinalChart] chartData.length:', chartData.length)
      ...
      <div className="p-4 border-b">
        ...
        <div className="flex gap-2 items-center">
          ...
          {process.env.NODE_ENV === 'development' && (
            <button
              onClick={() => {
                console.log('[FinalChart] Manual update triggered')
                initAttemptsRef.current = 0
