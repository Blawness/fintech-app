...
interface UltraOptimizedPortfolioProps {
  userId: string
  className?: string
}

export function UltraOptimizedPortfolio({ userId, className = '' }: UltraOptimizedPortfolioProps) {
  ...
  // Ultra smart polling - much less frequent
  const getUpdateInterval = useCallback(() => {
    if (!isUserActiveRef.current) return 300000 // 5 minutes when inactive
    if (retryCountRef.current === 0) return 120000 // 2 minutes normal
    if (retryCountRef.current === 1) return 300000 // 5 minutes after error
    if (retryCountRef.current === 2) return 600000 // 10 minutes after 2 errors
    return 900000 // 15 minutes after 3+ errors
  }, [])
  ...
  useEffect(() => {
    ...
    const checkUserActivity = () => {
      ...
      isUserActiveRef.current = timeSinceActivity < 300000 // 5 minutes
    }
    ...
    const activityCheckInterval = setInterval(checkUserActivity, 60000)
  ...
  // Ultra optimized fetch function
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      ...
      if (isVisibleRef.current && isUserActiveRef.current) {
        // Only fetch if user is active and tab becomes visible
        fetchPortfolio(true)
      }
    }
  ...
  // Ultra smart polling with user activity awareness
  useEffect(() => {
    // Initial fetch
    fetchPortfolio(true)

    // Set up ultra smart polling
    const scheduleNextUpdate = () => {
      ...
      updateIntervalRef.current = setTimeout(() => {
        if (isVisibleRef.current && isUserActiveRef.current) {
          ...
          // Schedule check again in 1 minute if inactive
          updateIntervalRef.current = setTimeout(scheduleNextUpdate, 60000)
        }
      }, interval)
    }
