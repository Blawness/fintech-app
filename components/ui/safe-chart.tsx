...
const SafeChart: React.FC<SafeChartProps> = ({ product, className = '' }) => {
  ...
  const fetchChartData = useCallback(async (hours: number) => {
    ...
    try {
      console.log(`[SafeChart] Fetching data for ${product.name}, hours: ${hours}`)
      ...
      console.log(`[SafeChart] Data received:`, result.data.chartData.length, 'points')
      setChartData(result.data.chartData)
      
    } catch (err) {
      console.error('[SafeChart] Fetch error:', err)
  ...
  const safeCleanup = useCallback(() => {
    if (isCleaningUpRef.current) {
      console.log('[SafeChart] Already cleaning up, skipping')
      return
    }

    isCleaningUpRef.current = true
    console.log('[SafeChart] Starting safe cleanup...')

    try {
      if (chartRef.current) {
        console.log('[SafeChart] Removing chart...')
      ...
      console.log('[SafeChart] Cleanup completed successfully')
      
    } catch (err) {
      console.error('[SafeChart] Cleanup error (non-critical):', err)
    } finally {
      isCleaningUpRef.current = false
    }
  }, [])

  // Initialize chart with safe DOM handling
  const initializeChart = useCallback(async () => {
    console.log('[SafeChart] === INITIALIZE START ===')
    
    if (isInitializingRef.current) {
      console.log('[SafeChart] Already initializing, skipping')
      return
    }

    if (chartRef.current) {
      console.log('[SafeChart] Chart already exists, skipping')
      return
    }

    if (!chartContainerRef.current) {
      console.error('[SafeChart] No container found')
      return
    }
    ...
    try {
      console.log('[SafeChart] Starting chart initialization...')
      ...
      try {
        ...
        console.warn('[SafeChart] Container clear warning:', clearError)
        // Continue anyway
      }
      ...
      console.log('[SafeChart] Creating chart with dimensions:', container.clientWidth, 'x', container.clientHeight)
      ...
      console.log('[SafeChart] Chart created successfully')
      chartRef.current = chart

      // Add line series using correct v5.0.8 API
      console.log('[SafeChart] Adding line series...')
      try {
        ...
        console.log('[SafeChart] Line series created successfully')
        lineSeriesRef.current = lineSeries
        
      } catch (seriesError) {
        console.error('[SafeChart] Error creating line series:', seriesError)
        throw seriesError
      }
      ...
      const handleResize = () => {
        if (chartContainerRef.current && chartRef.current) {
          try {
            ...
            console.warn('[SafeChart] Resize warning:', resizeError)
          }
        }
      }
      ...
      console.log('[SafeChart] === INITIALIZE COMPLETE ===')

    } catch (err) {
      console.error('[SafeChart] Initialize error:', err)
  ...
  const updateChart = useCallback(() => {
    if (!lineSeriesRef.current || chartData.length === 0) {
      console.log('[SafeChart] Cannot update - series or data not ready')
      return
    }

    try {
      console.log('[SafeChart] Updating chart with', chartData.length, 'data points')
      ...
      console.log('[SafeChart] Valid data points:', uniqueData.length)

      if (uniqueData.length === 0) {
        console.warn('[SafeChart] No valid data to display')
        return
      }
      ...
      console.log('[SafeChart] Chart updated successfully')
      
    } catch (err) {
      console.error('[SafeChart] Update error:', err)
  ...
  useEffect(() => {
    console.log('[SafeChart] === MOUNT EFFECT ===')
  ...
  useEffect(() => {
    console.log('[SafeChart] === DATA UPDATE EFFECT ===')
    console.log('[SafeChart] isChartReady:', isChartReady)
    console.log('[SafeChart] chartData.length:', chartData.length)
    ...
    <div className={`bg-white rounded-lg border ${className}`}>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="p-2 bg-gray-100 text-xs text-gray-600 border-b">
          ...
          Initializing: {isInitializingRef.current ? 'Yes' : 'No'}
        </div>
      )}
      ...
      <div className="p-4 border-b">
        ...
        <div className="flex gap-2 items-center">
          ...
          {process.env.NODE_ENV === 'development' && (
            <button
              onClick={() => {
                console.log('[SafeChart] Manual update triggered')
