...
const MinimalChart: React.FC<MinimalChartProps> = ({ product, className = '' }) => {
  ...
  // Initialize chart - minimal and safe approach
  ...
  const updateChart = useCallback(() => {
    if (!lineSeriesRef.current || chartData.length === 0) {
      console.log('[MinimalChart] Cannot update - series or data not ready')
      return
    }
  ...
  const cleanupChart = useCallback(() => {
    if (chartRef.current) {
      try {
        ...
        console.error('[MinimalChart] Cleanup error:', err)
      }
    }
  }, [])
